...
export class DrillDownColumnBarComponent implements OnInit {
  ...
  setAxis() {
    ...
    const label = this.categoryAxis.renderer.labels.template;
    label.truncate = true;
    label.maxWidth = 300;
  ...
  setSeries() {
    ...
    this.series.columns.template.events.on('hit', (ev: any) => {
      ...
      if (!(this.actualDrillDown >= this.drillDowns.length)) {
        const actualData = this.drillDowns[this.actualDrillDown].data.filter(
          (obj) => {
        ...
        if (this.actualDrillDown === 0) {
          ...
          this.actualNavName =
            ev.target.dataItem.dataContext[
              this.drillDowns[this.actualDrillDown - 1].campo
            ];
        }
        this.nav.data.push({
  ...
  drillDownFilter(obj: any, dataContext: any) {
    if (obj[this.category] === dataContext[this.category]) {
      for (let index = 0; index < this.actualDrillDown; index++) {
        if (
          obj[this.drillDowns[index].campo] !=
          dataContext[this.drillDowns[index].campo]
        ) {
          return false;
        }
      }
    ...
    this.nav.links.template.events.on('hit', (ev: any) => {
      ...
      if (target.step) {
        ...
        for (let index = 0; index < this.chart.series.values.length; index++) {
          this.chart.series.values[
            index
          ].dataFields.categoryX = this.drillDowns[this.actualDrillDown].campo;
        }
        this.categoryAxis.title.text = this.drillDowns[
          this.actualDrillDown
        ].campo;
        this.categoryAxis.dataFields.category = this.drillDowns[
          this.actualDrillDown
        ].campo;
        this.actualDrillDown++;
  ...
  setMultipleSeries() {
    ...
    for (let i = 0; i < this.chart.series.length; i++) {
      this.chart.series.values[i].columns.template.events.on(
        'hit',
        (ev: any) => {
          this.chart.colors.reset();

          if (!(this.actualDrillDown >= this.drillDowns.length)) {
            const actualData = this.drillDowns[
              this.actualDrillDown
            ].data.filter((obj) => {
              if (this.drillDownFilter(obj, ev.target.dataItem.dataContext)) {
                return obj;
            ...
            for (let index = 0; index < this.chart.series.length; index++) {
              this.chart.series.values[
                index
              ].dataFields.categoryX = actualCategory;
            }
            ...
            if (this.actualDrillDown === 0) {
              this.actualNavName =
                ev.target.dataItem.dataContext[this.category];
            } else {
              this.actualNavName =
                ev.target.dataItem.dataContext[
                  this.drillDowns[this.actualDrillDown - 1].campo
